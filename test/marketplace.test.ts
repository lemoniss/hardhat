...
//
//     // 컨트랙트 배포
//     const MusitNFT = await ethers.getContractFactory("MillimxNft");
//     const Marketplace = await ethers.getContractFactory("Marketplace");
//     millimxNft = await MusitNFT.deploy(mintPrice);
...
//       await millimxNft.connect(addr1).approve(marketplace.address, 1);
//     });
//
...
//         marketplace
//           .connect(addr1)
//           .enroll(millimxNft.address, 1, ethToWei(price))
//       )
//         .to.emit(marketplace, "Enrolled")
//         .withArgs(1, 1, ethToWei(price), addr1.address, millimxNft.address);
//
//       expect(await millimxNft.ownerOf(1)).to.equal(marketplace.address);
//
...
//       expect(item.nft).to.equal(millimxNft.address);
...
//       price = 0;
//       await expect(
//         marketplace.connect(addr1).enroll(millimxNft.address, 1, 0)
...
//       );
//
//       expect(await millimxNft.ownerOf(1)).to.equal(addr2.address);
