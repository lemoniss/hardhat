...
// import { Auction, MillimxNft } from "../typechain";
//
...
//     millimxNft = await MillimxNft.deploy(mintPrice);
//     auction = await Auction.deploy(feePercent);
//   })
//
//   describe("Deployment", async () => {
//     it("MillimxNft Contract's name and symbol test", async () => {
...
//
//     beforeEach(async ()=> {
//       await millimxNft.connect(addr1).minting(tokenURI, {value : mintPrice})
//       await millimxNft.connect(addr1).approve(auction.address, 1);
//       startAt = Date.now()
//       endAt =  startAt + 60000 // 시작후 1분 뒤 종료
//       nft = millimxNft.address;
//     })
//
...
//           .withArgs(1, startPrice, millimxNft.address, 1, addr1.address);
//
